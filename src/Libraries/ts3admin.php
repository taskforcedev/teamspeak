<?php namespace Taskforcedev\Teamspeak\Libraries;

/**
 * TS3 Admin class by Par0noid.
 * This file has been modified (tidied up) by Taskforcedev to reduce file clutter
 * and to match modern coding standards (PSR-2).
 *
...
class ts3admin
{
    ...
    /**
     * runtime is a private handle and configuration storage
     */
     ...
     * providing the three parameters of each permission. A permission can be specified by permid or permsid.
     *
     * <b>Input-Array like this:</b>
     * <pre>
     * $permissions = array();
     ...
     * channelCreate
     *
     * Creates a new channel using the given properties and displays its ID. Note that this command accepts multiple
     * properties which means that you're able to specifiy all settings of the new channel at once.
     *
     ...
     * channelDelete
     *
     * Deletes an existing channel by ID. If force is set to 1, the channel will be deleted even if there are clients
     * within. The clients will be kicked to the default channel with an appropriate reason message.
     *
     * @param        integer $cid   channelID
     ...
     * channelDelPerm
     *
     * Removes a set of specified permissions from a channel. Multiple permissions can be removed at once. A permission
     ...
     * channelGroupAdd
     *
     * Creates a new channel group using a given name and displays its ID. The optional type parameter can be used to
     * create ServerQuery groups and template groups.
     *
     * <b>groupDbTypes:</b>
     *    <ol start="0">
     ...
     * channelGroupAddPerm
     *
     * Adds a set of specified permissions to a channel group. Multiple permissions can be added by providing the two
     * parameters of each permission. A permission can be specified by permid or permsid.
     *
     * <b>Input-Array like this:</b>
     * <pre>
     ...
     * optional so you're free to choose the most suitable combination for your requirement
     *
     * <b>Output:</b>
     * <pre>
     * Array
     ...
     * channelGroupCopy
     *
     * Creates a copy of the channel group specified with scgid. If tcgid is set to 0, the server will create a new
     * group. To overwrite an existing group, simply set tcgid to the ID of a designated target group. If a target
     * group is set, the name parameter will be ignored. The type parameter can be used to create ServerQuery groups
     * and template groups.
     *
     * <b>groupDbTypes:</b>
     *    <ol start="0">
     ...
     * channelGroupDelete
     *
     * Deletes a channel group by ID. If force is set to 1, the channel group will be deleted even if there are clients
     * within.
     *
     * @param        integer $cgid  channelGroupID
     ...
     * channelGroupDelPerm
     *
     * Removes a set of specified permissions from the channel group. Multiple permissions can be removed at once. A
     ...
     * channelList
     *
     * Displays a list of channels created on a virtual server including their ID, order, name, etc. The output can be
     ...
     * channelMove
     *
     * Moves a channel to a new parent channel with the ID cpid. If order is specified, the channel will be sorted
     * right under the channel with the specified ID. If order is set to 0, the channel will be sorted right below the
     * new parent.
     *
     * @param        integer $cid   channelID
     ...
     * clientDelPerm
     *
     * Removes a set of specified permissions from a client. Multiple permissions can be removed at once. A permission
     * can be specified by permid or permsid.
     *
     * <b>Input-Array like this:</b>
     * <pre>
     ...
     * clientKick
     *
     * Kicks one or more clients specified with clid from their currently joined channel or from the server, depending
     * on reasonid. The reasonmsg parameter specifies a text message sent to the kicked clients. This parameter is
     ...
     * clientList
     *
     * Displays a list of clients online on a virtual server including their ID, nickname, status flags, etc. The
     * output can be modified using several command options. Please note that the output will only contain clients
     ...
     * complainList
     *
     * Displays a list of complaints on the selected virtual server. If tcldbid is specified, only complaints about the
     * targeted client will be shown.
     *
     * <b>Output:</b>
     * <pre>
     * Array
     ...
     * client-side. On success, the server generates a new ftkey which is required to start downloading the file
     ...
     * On success, the server generates a new ftkey which is required to start uploading the file through TeamSpeak 3's
     ...
     * ftList
     *
     * Displays a list of running file transfers on the selected virtual server. The output contains the path to which
     * a file is uploaded to, the current transfer rate in bytes per second, etc
     *
     * <b>Output:</b>
     * <pre>
     * Array
     ...
     * ftRenameFile
     *
     * Renames a file in a channels file repository. If the two parameters tcid and tcpw are specified, the file will
     * be moved into another channels file repository.
     *
     * @param        integer $cid     channelID
     * @param        string  $cpw     channelPassword (leave blank if not needed)
     ...
     * hostInfo
     *
     * Displays detailed connection information about the server instance including uptime, number of virtual servers
     * online, traffic information, etc.
     *
     * <b>Output:</b>
     * <pre>
     * Array
     ...
     * logView
     *
     * Displays a specified number of entries from the servers log. If instance is set to 1, the server will return
     * lines from the master logfile (ts3server_0.log) instead of the selected virtual server logfile.
     *
     * <b>Output:</b>
     * <pre>
     * Array
     ...
     * }
     * {
     *  [l] => 2012-01-10 20:34:31.380260|INFO    |DatabaseQuery |   | dbPlugin name:    SQLite3 plugin, Version 2,
     ...
     * permOverview
     *
     * Displays all permissions assigned to a client for the channel specified with cid. If permid is set to 0, all
     * permissions will be displayed. A permission can be specified by permid or permsid.
     *
     * <b>Output:</b>
     * <pre>
     * Array
     ...
     * selectServer
     *
     * Selects the virtual server specified with sid or port to allow further interaction. The ServerQuery client will
     * appear on the virtual server and acts like a real TeamSpeak 3 Client, except it's unable to send or receive
     * voice data. If your database contains multiple virtual servers using the same UDP port, use will select a random
     * virtual server using the specified port.
     *
     ...
     *             http://forum.teamspeak.com/showthread.php/84280-Sendtextmessage-by-query-client
     * @return     boolean    success
     */
     ...
     * serverCreate
     *
     * Creates a new virtual server using the given properties and displays its ID, port and initial administrator
     * privilege key. If virtualserver_port is not specified, the server will test for the first unused UDP port. The
     * first virtual server will be running on UDP port 9987 by default. Subsequently started virtual servers will be
     * running on increasing UDP port numbers.
     *
     * <b>Input-Array like this:</b>
     * <pre>
     * $data = array();
     ...
     * serverDelete
     *
     * Deletes the virtual server specified with sid. Please note that only virtual servers in stopped state can be
     * deleted.
     *
     ...
     * serverGroupAdd
     *
     * Creates a new server group using the name specified with name and displays its ID. The optional type parameter
     * can be used to create ServerQuery groups and template groups. For detailed information, see
     *
     * <b>Output:</b>
     * <pre>
     * Array
     ...
     * optional -names option, the output will also contain the last known nickname and the unique identifier of the
     * clients.
     *
     ...
     * serverGroupCopy
     *
     * Creates a copy of the server group specified with ssgid. If tsgid is set to 0, the server will create a new
     * group. To overwrite an existing group, simply set tsgid to the ID of a designated target group. If a target
     ...
     * serverGroupDelete
     *
     * Deletes the server group specified with sgid. If force is set to 1, the server group will be deleted even if
     ...
     * removed at once. A permission can be specified by permid or permsid.
     *
     * <b>Input-Array like this:</b>
     * <pre>
     * $permissions = array();
     ...
     * serverGroupPermList
     *
     * Displays a list of permissions assigned to the server group specified with sgid. If the permsid option is
    ...
    /**
     * Displays a list of virtual servers including their ID, status, number of clients online, etc. If you're using
     * the -all option, the server will list all virtual servers stored in the database. This can be useful when
     * multiple server instances with different machine IDs are using the same database. The machine ID is used to
     * identify the server instance a virtual server is associated with. The status of a virtual server can be either
     * online, offline, deploy running, booting up, shutting down and virtual online. While most of them are
     * self-explanatory, virtual online is a bit more complicated. Please note that whenever you select a virtual
     * server which is currently stopped, it will be started in virtual mode which means you are able to change its
     * configuration, create channels or change permissions, but no regular TeamSpeak 3 Client can connect. As soon as
     * the last ServerQuery client deselects the virtual server, its status will be changed back to offline.
     *
     ...
     * The data from a server snapshot can be used to restore a virtual servers configuration, channels and permissions
     ...
     * Please note that the TeamSpeak 3 Server does NOT check for necessary permissions while deploying a snapshot so
     * the command could be abused to gain additional privileges.
     *
    ...
    /**
     * Sets a new temporary server password specified with pw. The temporary password will be valid for the number of
     * seconds specified with duration. The client connecting with this password will automatically join the channel
     * specified with tcid. If tcid is set to 0, the client will join the default channel.
     *
    ...
    /**
     * Returns a list of active temporary server passwords. The output contains the clear-text password, the nickname
     * and unique identifier of the creating client.
     *
     * <b>Output:</b>
     * <pre>
     * Array
     ...
     * Otherwise, tokenid1 is used as a channel group ID and you need to provide a valid channel ID using tokenid2. The
     * tokencustomset parameter allows you to specify a set of custom client properties. This feature can be used when
     * generating tokens to combine a website account database with a TeamSpeak user. The syntax of the value needs to
     * be escaped using the ServerQuery escape patterns and has to follow the general syntax of: ident=ident1
     * value=value1|ident=ident2 value=value2|ident=ident3 value=value3
     *
     * <b>Input-Array like this:</b>
     * <pre>
    ...
    /**
     * Displays a list of privilege keys available including their type and group IDs. Tokens can be used to gain
     * access to specified server or channel groups. A privilege key is similar to a client with administrator
     * privileges that adds you to a certain permission group, but without the necessity of a such a client with
     * administrator privileges to actually exist. It is a long (random looking) string that can be used as a ticket
    ...
    /**
     * Use a token key gain access to a server or channel group. Please note that the server will automatically delete
     * the token after it has been used.
     *
     * @param        string $token token
     *
     * @return     boolean success
